...
{
    public partial class VsAIForm : MetroForm
    {
        // alpha beta pruning 알고리즘을 이용한 AI입니다.(적대 탐색 알고리즘이라고도 불림);
        ...
        private int targetX, targetY; // AI가 선택하기 위한 X축, Y축
        private int limit = 1; //한수 앞선 수를 알아본다.(Alpha Beta 알고리즘 사용) 그런데, 여기서 연산이 복잡해서 두개이상이면 터짐. 보완점.
        ...
        {
            if (nowPlayer == aiPlayer)  //ai가 하고있으면 놓는게 불가능
                return;
            ...
            if (judge())
            {
                if (nowPlayer == aiPlayer) status.Text = "AI 플레이어의 승리";
                status.Text = "승리!!";
        ...
        #region 오목의 경우의수(AI를 위한)
        /*------------- 5목이 만들어질 수 있는 모든 20가지 경우의 수. 이건 기본적으로 단순하기 때문에 다 적어줌. -------------*/
        ...
        #region AI의 생각(위치의 가치를 구해서, 가장 높은 위치에 두기위한 가치책정)
